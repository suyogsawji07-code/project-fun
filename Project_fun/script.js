...
/* ================= FRIEND REQUEST PAGE: dodging No button ================= */
function initFriendRequest() {
  const stage = document.getElementById('friendStage');
  const noBtn = document.getElementById('noBtn');
  const yesBtn = document.getElementById('yesBtn');
  const msg = document.getElementById('dodgeMsg');
  const celebration = document.getElementById('yesCelebration');
  const btnsRow = document.getElementById('friendBtns');
  if (!stage || !noBtn || !yesBtn) return;

  const dodgeLines = [
    "Nice try 😏", "Nope, not today 😂", "Too slow! 🐾",
    "You can't catch me 🏃", "Wrong answer 🙅", "Try again 💛",
    "Almost! 😜", "Keep trying 🫶", "This button knows the answer 😌"
  ];
  let dodgeCount = 0;
  ...
  function moveNoButton() {
    ...
    if (!noBtn.classList.contains('roaming')) {
      ...
      btnsRow.style.minHeight = stage.offsetHeight + 'px';
    }
    noBtn.style.left = newX + 'px';
    noBtn.style.top = newY + 'px';

    dodgeCount++;
    if (msg) msg.textContent = dodgeLines[Math.min(dodgeCount - 1, dodgeLines.length - 1)];
    noBtn.style.transform = `scale(${0.92 + Math.random() * 0.16})`;
  }
  ...
  yesBtn.addEventListener('click', () => {
    ...
    if (window.launchFireworks) launchFireworks(4);
  });
}
document.addEventListener('DOMContentLoaded', initFriendRequest);
